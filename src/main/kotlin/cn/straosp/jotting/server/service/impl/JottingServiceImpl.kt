...
class JottingServiceImpl : JottingService {
    ...
    override fun getJottingByDate(accountID: Int, localDate: LocalDate): Result<List<JottingReminder>> {
        val result = AppDatabase.database.from(JottingTable)
            .leftJoin(ReminderTable, on = JottingTable.reminderID eq ReminderTable.id)
            .select(
                JottingTable.id,
            ...
            .where {
                JottingTable.accountID eq accountID and (JottingTable.recordDate.between(LocalDateRange(localDate.atStartOfDay(),localDate.plusDays(1).atStartOfDay())))
            }
            .orderBy(JottingTable.recordDate.desc())
            .map { row ->
                JottingReminder(
